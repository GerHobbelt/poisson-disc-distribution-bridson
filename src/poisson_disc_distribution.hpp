...
#ifndef PDDB_POISSON_DISC_DISTRIBUTION_HPP
#define PDDB_POISSON_DISC_DISTRIBUTION_HPP
...
//   fault.
    ...
    poisson_disc_distribution(config conf, T&& random, T2&& in_area, T3&& output)
    {
        ...
        auto add = [&process, &output, &set](const point& p, const int width) {
            process.push(p);
            output(p, width);
            set(p);
        };
        ...
        if (conf.start.x == infinity) {
            ...
            } while (!in_area(conf.start, conf));
        }

        add(conf.start, int(conf.width));
        ...
        while (!process.empty()) {
            ...
            for (int i = 0; i != conf.max_attempts; ++i) {
                ...
                if (in_area(p, conf) && !point_too_close(p)) {
                    add(p, int(conf.width));
                }
            }
        }
    }
...
#endif //PDDB_POISSON_DISC_DISTRIBUTION_HPP
